...
NSString *const kGPUImageLensTiltedTexelSamplingVertexShaderString = SHADER_STRING
(
...
NSString *const kGPUImageLensMotionBlurFragmentShaderString = SHADER_STRING
(
 ...
 void main()
 {
     ...
     mediump vec4 fragmentColor = exp(4.0 * texture2D(inputImageTexture, textureCoordinate)) * 0.18;
     fragmentColor += exp(4.0 * texture2D(inputImageTexture, oneStepBackTextureCoordinate)) * 0.15;
     fragmentColor += exp(4.0 * texture2D(inputImageTexture, twoStepsBackTextureCoordinate)) *  0.12;
     fragmentColor += exp(4.0 * texture2D(inputImageTexture, threeStepsBackTextureCoordinate)) * 0.09;
     fragmentColor += exp(4.0 * texture2D(inputImageTexture, fourStepsBackTextureCoordinate)) * 0.05;
     fragmentColor += exp(4.0 * texture2D(inputImageTexture, oneStepForwardTextureCoordinate)) * 0.15;
     fragmentColor += exp(4.0 * texture2D(inputImageTexture, twoStepsForwardTextureCoordinate)) *  0.12;
     fragmentColor += exp(4.0 * texture2D(inputImageTexture, threeStepsForwardTextureCoordinate)) * 0.09;
     fragmentColor += exp(4.0 * texture2D(inputImageTexture, fourStepsForwardTextureCoordinate)) * 0.05;
     
     gl_FragColor = log(fragmentColor) / 4.0;
 }
 );

@interface GPUImageLensMotionBlurFilter()
{
...
@implementation GPUImageLensMotionBlurFilter
...
- (id)init;
{
    if (!(self = [super initWithVertexShaderFromString:kGPUImageLensTiltedTexelSamplingVertexShaderString fragmentShaderFromString:kGPUImageLensMotionBlurFragmentShaderString]))
    {
        return nil;
    }
