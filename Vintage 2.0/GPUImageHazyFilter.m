...
@implementation GPUImageHazyFilter
...
- (NSString *)fragmentShaderForOptimizedBlurOfRadius:(NSUInteger)blurRadius sigma:(CGFloat)sigma;
{
    ...
    [shaderString appendFormat:@"\
     ...
     \n\
     varying highp vec2 blurCoordinates[%lu];\n\
     mediump float opacity = %f;\n\
     \n\
     void main()\n\
     {\n\
     lowp vec4 pixel = texture2D(inputImageTexture, blurCoordinates[0]);\n\
     lowp vec4 sum = vec4(0.0);\n", (unsigned long)(1 + (numberOfOptimizedOffsets * 2)), _opacity];
#else
    [shaderString appendFormat:@"\
     uniform sampler2D inputImageTexture;\n\
     uniform float texelWidthOffset;\n\
     ...
     void main()\n\
     {\n\
     vec4 pixel = texture2D(inputImageTexture, blurCoordinates[0]);\n\
     vec4 sum = vec4(0.0);\n", 1 + (numberOfOptimizedOffsets * 2), _opacity];
#endif
    ...
    // Footer
    [shaderString appendString:@"\
     gl_FragColor = sum * opacity + (1.0 - opacity) * pixel;\n\
     }\n"];
